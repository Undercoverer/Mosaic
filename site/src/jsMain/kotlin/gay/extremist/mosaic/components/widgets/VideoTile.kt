...
fun VideoTile(onClick: () -> Unit, content: @Composable () -> Unit) {
    ...
    Row(Modifier.fillMaxSize().height(10.cssRem).padding(15.px)
        ...
        .background(Color.rgba(0.1f,0.1f,0.1f,0.6f))
        .onClick { onClick() }) {
        Box(Modifier.fillMaxWidth(20.percent).height(7.cssRem).outline(1.px, LineStyle.Solid), Alignment.Center) {
            ContentScale.Fit
        ...
        Box(Modifier.fillMaxWidth(80.percent).outline(1.px, LineStyle.Solid)) {
            content()
        }
    }
}

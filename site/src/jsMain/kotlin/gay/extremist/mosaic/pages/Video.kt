                        ...
                        key(followedAccounts){
                            if (video.creator in followedAccounts)  {
                                Button(onClick = {
                                    coroutineScope.launch{
                                        postRequestText(
                                            urlString = "accounts/${video.creator.id}/unfollow",
                                            setHeaders = {
                                ...
                                if (isDeleteVisible){
                                    Popover(
                                        ...
                                        content = {
                                            Column(Modifier.fillMaxWidth().gap(1.cssRem), horizontalAlignment = Alignment.CenterHorizontally) {
                                ...
                                Button(onClick = {
                                    coroutineScope.launch{
                                        postRequestText(
                                            urlString = "accounts/${video.creator.id}/follow",
                                            setHeaders = {
                                                append(headerAccountId, window.localStorage["id"] ?: "")
                                                append(headerToken, window.localStorage["token"] ?: "")
                                            },
                        ...
                        Box(Modifier.fillMaxSize().padding(2.cssRem).height(15.cssRem).overflow { y(Overflow.Auto) }, Alignment.TopCenter) {
                            Column(Modifier.gap(1.cssRem).fillMaxSize()) {
                                CommentFunc { comment ->
                                    ...
                                    coroutineScope.launch{
                                        newComment = postRequest<String, CommentResponseNonrecursive>(
                                            urlString = "comments/video/$id",
                                            setHeaders = {
                                                append(headerAccountId, window.localStorage["id"] ?: "")
                                                append(headerToken, window.localStorage["token"] ?: "")
                    ...
                    Box(Modifier.fillMaxSize().padding(2.cssRem).height(60.cssRem).overflow { y(Overflow.Auto) }, Alignment.TopCenter) {
                        Column(Modifier.gap(1.cssRem).fillMaxSize()){
                            for (recommend in (recommends.filter { video.id != it.id })) {
                                if(id != recommend.id){
                                    VideoTile(
                                        onClick = { pageCtx.router.tryRoutingTo("/video/${recommend.id}") },
                                        recommend
                                    )
                                }
                            }
